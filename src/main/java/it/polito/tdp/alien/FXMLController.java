...
private Dizionario model;
    ...
    if(s.length()==0) {
    	txtTraduzione.setText("Errore inserire una parola");
    		...
    		if(alieno.matches("[a-z]+")==false) {
    			txtTraduzione.setText("Errore, scrivere nel formato richiesto");
    			}
    		}
    else if(!(s.contains(" "))) {
    		...
    		//else
    		//	txtTraduzione.setText("Errore nell'inserimento");
    	//}
    	//else {
    		//String s= model.traduzione(p);
    		//List<String> s=model.traduzioni(p);
    		//txtTraduzione.appendText(s.toString());
    	//}
    	//}
